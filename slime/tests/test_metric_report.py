...
@pytest.mark.unit
def test_train_step_metric_reducer_supports_ratio_and_extrema(mock_dp_with_cp_group):
    """OPD clip ratio uses its own token-entry denominator; extrema are global."""
    reduced = reduce_train_step_metrics(
        [
            {
                "keys": ["mean", "ratio", "minimum", "maximum"],
                "values": torch.tensor([0.0, 2.0, 1.0, -3.0, 5.0]),
                "metric_reduce_ops": torch.tensor([0, 3, 1, 2]),
                "metric_reduce_denominators": torch.tensor([0.0, 2.0, 0.0, 0.0]),
            },
            {
                "keys": ["mean", "ratio", "minimum", "maximum"],
                "values": torch.tensor([0.0, 4.0, 1.0, -5.0, 4.0]),
                "metric_reduce_ops": torch.tensor([0, 3, 1, 2]),
                "metric_reduce_denominators": torch.tensor([0.0, 3.0, 0.0, 0.0]),
            },
        ],
        calculate_per_token_loss=False,
    ...
    assert reduced["mean"] == pytest.approx(2.0)
    assert reduced["ratio"] == pytest.approx(0.4)
    assert reduced["minimum"] == pytest.approx(-5.0)
    assert reduced["maximum"] == pytest.approx(5.0)


@pytest.mark.unit
def test_train_step_metric_reducer_reports_exact_token_kl_variance_quantiles(
    monkeypatch, mock_dp_with_cp_group
):
    import torch.distributed as dist

    monkeypatch.setattr(dist, "get_world_size", lambda group=None: 1)
    sequence_variances = torch.tensor([0.0, 1.0, 4.0, 9.0])
    reduced = reduce_train_step_metrics(
        [
            {
                "keys": ["loss"],
                "values": torch.tensor([0.0, 1.0]),
                OPD_TOKEN_KL_VARIANCES_KEY: sequence_variances[:2],
            },
            {
                "keys": ["loss"],
                "values": torch.tensor([0.0, 1.0]),
    ...
    assert reduced["opd_token_kl_variance_min"] == pytest.approx(0.0)
    assert reduced["opd_token_kl_variance_max"] == pytest.approx(9.0)
    assert reduced["opd_token_kl_variance_p05"] == pytest.approx(torch.quantile(sequence_variances, 0.05).item())
    assert reduced["opd_token_kl_variance_p95"] == pytest.approx(torch.quantile(sequence_variances, 0.95).item())
