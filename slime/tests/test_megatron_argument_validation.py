...
@pytest.mark.unit
def test_hf_validate_dense_qwen3_5_ignores_inherited_moe_defaults(monkeypatch):
    module = load_arguments_module(monkeypatch)
    args = make_qwen3_6_args(
    ...
    text_config = types.SimpleNamespace(
        ...
        # These are inherited defaults in SGLang's dense compatibility class.
        moe_intermediate_size=512,
        shared_expert_intermediate_size=512,
        num_experts=512,
        tie_word_embeddings=False,
    ...
    module._hf_validate_args(args, types.SimpleNamespace(text_config=text_config))
...
@pytest.mark.unit
def test_topk_opd_requires_sglang_teacher(monkeypatch):
    module = load_slime_arguments_module(monkeypatch)
    args = make_slime_validate_args(use_opd=True, opd_type="megatron", opd_loss_type="topk")
    with pytest.raises(ValueError, match="requires --opd-type=sglang"):
        module.slime_validate_args(args)


@pytest.mark.unit
def test_topk_opd_requires_positive_k(monkeypatch):
    module = load_slime_arguments_module(monkeypatch)
    args = make_slime_validate_args(use_opd=True, opd_type="sglang", opd_loss_type="topk", opd_top_k=0)
