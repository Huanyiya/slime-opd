...
def test_opd_kl_estimators_match_formula_in_topk_and_sampled_paths(monkeypatch, opd_kl_loss_type):
    ...
    try:
        ...
        student_logq = torch.log_softmax(student_log_probs, dim=-1)
        teacher_logq = torch.log_softmax(teacher_log_probs, dim=-1)
        student_q = student_logq.exp()
        expected_token_kls = (student_q * (student_logq - teacher_logq)).sum(-1)
        reference_topk_loss = expected_token_kls.mean()
...
@pytest.mark.unit
def test_topk_detatch_uses_rollout_logprobs_for_detached_ppo_advantage(monkeypatch):
    previous_loss = sys.modules.pop("slime.backends.megatron_utils.loss", None)
    previous_cp_utils = sys.modules.pop("slime.backends.megatron_utils.cp_utils", None)
    mpu_stub = types.SimpleNamespace(
        get_context_parallel_world_size=lambda: 1,
    ...
    try:
        ...
        logits = torch.tensor(
            [[[0.0, 0.0, 0.0], [0.5, -0.5, 1.0], [-1.0, 2.0, 0.0]]],
            dtype=torch.float32,
            requires_grad=True,
        )
        topk_ids = torch.tensor([[2, 0], [1, 2]], dtype=torch.long)
        # Deliberately differs from both teacher and current actor values. If
        # the rollout values are overwritten by current logprobs, this formula
        # and the gradient no longer match.
        old_logp = torch.tensor([[-1.7, -2.0], [-0.3, -2.4]], requires_grad=True)
        teacher_logp = torch.tensor([[-0.2, -2.6], [-1.4, -0.9]], requires_grad=True)
        batch = {
            "unconcat_tokens": [torch.tensor([10, 11, 12, 13])],
            "total_lengths": [4],
            "response_lengths": [2],
            ...
            "rollout_mask_sums": torch.tensor([2.0]),
            "opd_topk_token_ids": [topk_ids],
            "opd_topk_rollout_log_probs": [old_logp],
            "opd_topk_teacher_log_probs": [teacher_logp],
        }

        loss, _normalizer, metrics = loss_function(
            args, batch, num_microbatches=1, step_global_batch_size=1, logits=logits
        )

        reference_logits = logits.detach().clone().requires_grad_()
        current_logp = torch.log_softmax(reference_logits[0, 1:3], dim=-1).gather(-1, topk_ids)
        with torch.no_grad():
            weights = torch.softmax(old_logp.detach(), dim=-1)
            advantage = -weights * (old_logp.detach() - teacher_logp.detach())
        ratio = torch.exp(current_logp - old_logp.detach())
        expected_per_token = torch.maximum(
            -advantage * ratio,
            -advantage * ratio.clamp(1 - args.eps_clip, 1 + args.eps_clip),
        ).sum(dim=-1)
        reference_loss = args.opd_kl_coef * expected_per_token.mean()

        torch.testing.assert_close(loss, reference_loss, rtol=1e-6, atol=1e-7)
        assert metrics["keys"] == ["loss", "opd_topk_detatch_loss", "student_entropy"]
        loss.backward()
        reference_loss.backward()
        torch.testing.assert_close(logits.grad, reference_logits.grad, rtol=1e-5, atol=1e-7)
        assert old_logp.grad is None
