...
@pytest.mark.unit
def test_process_group_cycle_can_be_disabled(monkeypatch):
    events = []
    monkeypatch.setenv("SLIME_RELOAD_PROCESS_GROUPS", "0")
    monkeypatch.setattr(
        rpg.ReloadableProcessGroup,
        "destroy_process_groups",
        staticmethod(lambda: events.append("destroy")),
    )
    monkeypatch.setattr(
        rpg.ReloadableProcessGroup,
        "reload_process_groups",
        staticmethod(lambda: events.append("reload")),
    )
    ...
    assert events == []
    assert not rpg.process_group_reloading_enabled()
