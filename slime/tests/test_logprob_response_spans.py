...
from slime.backends.megatron_utils.loss import (
    _build_response_aligned_topk_ids,
...
@pytest.mark.parametrize(
    ...
    [
        (0, [[107, 207]]),
        (1, [[104, 204], [105, 205], [106, 206]]),
    ],
)
def test_topk_ids_align_with_zigzag_cp_response_rows(monkeypatch, rank, expected):
    _set_cp(monkeypatch, size=2, rank=rank)
    aligned = _build_response_aligned_topk_ids(
        4,
        torch.device("cpu"),
        [torch.tensor([[104, 204], [105, 205], [106, 206], [107, 207]])],
        total_lengths=[8],
        response_lengths=[4],
        allgather_cp=False,
    )
    response_ids, _ = _extract_per_sample(aligned, None, [8], [4], allgather_cp=False)
    assert response_ids[0].tolist() == expected
...
def test_topk_ids_align_with_allgather_cp_response_rows(monkeypatch, rank, expected):
    _set_cp(monkeypatch, size=2, rank=rank)
    aligned = _build_response_aligned_topk_ids(
        3,
        torch.device("cpu"),
        [torch.tensor([[102, 202], [103, 203], [104, 204], [105, 205]])],
        total_lengths=[6],
        response_lengths=[4],
        allgather_cp=True,
    )
    response_ids, _ = _extract_per_sample(aligned, None, [6], [4], allgather_cp=True)
    assert response_ids[0].tolist() == expected
