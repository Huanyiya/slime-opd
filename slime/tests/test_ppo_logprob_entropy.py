...
@pytest.mark.parametrize("chunk_size", [-1, 1, 2, 8])
def test_calculate_selected_log_probs_matches_full_log_softmax(chunk_size: int):
    logits = _single_rank_logits().requires_grad_()
    target_ids = torch.tensor([[3, 1], [0, 2], [1, 3]], dtype=torch.long)

    selected_log_probs = calculate_selected_log_probs(
        logits,
        target_ids,
        tp_group=None,
        chunk_size=chunk_size,
    ...
    reference = torch.log_softmax(reference_logits, dim=-1).gather(-1, target_ids)
    torch.testing.assert_close(selected_log_probs, reference, rtol=STRICT_RTOL, atol=STRICT_ATOL)

    weights = torch.tensor([[0.2, -0.7], [1.1, 0.3], [-0.5, 0.9]], dtype=torch.float32)
    (selected_log_probs * weights).sum().backward()
    (reference * weights).sum().backward()
    torch.testing.assert_close(logits.grad, reference_logits.grad, rtol=0.0, atol=3e-8)


def test_calculate_selected_log_probs_handles_empty_input():
    logits = torch.empty((0, 4), dtype=torch.float32, requires_grad=True)
    target_ids = torch.empty((0, 3), dtype=torch.long)
    selected_log_probs = calculate_selected_log_probs(logits, target_ids, tp_group=None, chunk_size=2)
    assert selected_log_probs.shape == (0, 3)


@pytest.mark.parametrize("chunk_size", [-1, 1, 2, 8])
def test_calculate_selected_log_probs_entropy_matches_full_softmax(chunk_size: int):
    logits = _single_rank_logits().requires_grad_()
    target_ids = torch.tensor([[3, 1], [0, 2], [1, 3]], dtype=torch.long)

    selected_log_probs, entropy = calculate_selected_log_probs_and_entropy(
        logits,
        target_ids,
        tp_group=None,
        chunk_size=chunk_size,
    ...
    torch.testing.assert_close(selected_log_probs, reference_log_probs.gather(-1, target_ids))
...
def _distributed_selected_log_probs_worker(rank: int, world_size: int, master_port: int) -> None:
    import torch.distributed as dist
    ...
    try:
        full_logits = _distributed_full_logits()
        target_ids = torch.tensor([[5, 1], [0, 4], [3, 2], [2, 5]], dtype=torch.long)
        vocab_per_rank = full_logits.size(-1) // world_size
        vocab_start = rank * vocab_per_rank
        vocab_end = vocab_start + vocab_per_rank
        local_logits = full_logits[:, vocab_start:vocab_end].detach().clone().requires_grad_()

        selected, entropy = calculate_selected_log_probs_and_entropy(
            local_logits,
            target_ids,
            tp_group=None,
            chunk_size=2,
        )
        reference_logits = full_logits.detach().clone().requires_grad_()
        reference_log_probs = torch.log_softmax(reference_logits, dim=-1)
        reference = reference_log_probs.gather(-1, target_ids)
        reference_entropy = -(reference_log_probs.exp() * reference_log_probs).sum(dim=-1)
        torch.testing.assert_close(selected, reference, rtol=1e-6, atol=1e-7)
        torch.testing.assert_close(entropy, reference_entropy, rtol=1e-6, atol=1e-7)

        weights = torch.tensor([[0.2, -0.7], [1.1, 0.3], [-0.5, 0.9], [0.8, -0.2]])
        (selected * weights).sum().backward()
        (reference * weights).sum().backward()
        torch.testing.assert_close(
            local_logits.grad,
            reference_logits.grad[:, vocab_start:vocab_end],
            rtol=1e-6,
            atol=1e-7,
        )
    finally:
...
def test_calculate_selected_log_probs_vocab_parallel():
    import torch.multiprocessing as mp

    world_size = 2
    mp.spawn(
        _distributed_selected_log_probs_worker,
