...
class HfWeightIteratorDirect(HfWeightIteratorBase):
    def __init__(self, *args, target_device: torch.device | str | None = None, **kwargs):
        super().__init__(*args, **kwargs)
        self.target_device = torch.device(target_device) if target_device is not None else None
        # Cache topology while Megatron process groups are alive. Colocated
        # disk sync later runs after NCCL and its subgroups have been destroyed.
        self.parallel_sizes = {
            "pipeline": mpu.get_pipeline_model_parallel_world_size(),
            "expert_model": mpu.get_expert_model_parallel_world_size(),
...
def _get_megatron_full_params(
    ...
    parallel_sizes: dict[str, int] | None = None,
) -> Sequence[torch.Tensor]:
    monkey_patch_torch_reductions()
    pp_size = (
        parallel_sizes["pipeline"]
        if parallel_sizes is not None
        else mpu.get_pipeline_model_parallel_world_size()
    )
    ep_size = (
        parallel_sizes["expert_model"]
        if parallel_sizes is not None
        else mpu.get_expert_model_parallel_world_size()
    )
    rank = dist.get_rank()
    target_device = target_device or torch.device("cuda", torch.cuda.current_device())
    # init params:
    params = []
    for info in megatron_local_param_infos:
        if dist.get_rank() == info.src_rank:
            params.append(
                torch.nn.Parameter(
                    megatron_local_weights[info.name].to(device=target_device, non_blocking=True),
                    requires_grad=False,
                )
            )
        else:
            params.append(torch.empty(info.shape, dtype=info.dtype, device=target_device))
    ...
    gathered_params = all_gather_params_async(
        list(zip(megatron_local_param_infos, params, strict=False)),
        tensor_model_parallel_size=parallel_sizes["tensor"] if parallel_sizes is not None else None,
        expert_tensor_parallel_size=parallel_sizes["expert_tensor"] if parallel_sizes is not None else None,
    )
