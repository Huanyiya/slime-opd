...
def save_hf_model_to_path(
    ...
    *,
    ...
    hf_weight_iterator: Any | None = None,
) -> None:
    """Save a Megatron model as an HF checkpoint at a concrete directory."""
    if args.megatron_to_hf_mode == "bridge":
        if megatron_local_weights is not None or hf_weight_iterator is not None:
            raise ValueError("Megatron Bridge HF save does not support an explicit weight mapping or iterator.")
        ...
        save_hf_model_direct_to_path(
            ...
            hf_weight_iterator=hf_weight_iterator,
        )
...
def save_hf_model_direct_to_path(
    ...
    *,
    ...
    uses_explicit_cpu_weights = megatron_local_weights is not None
    if hf_weight_iterator is None:
        hf_weight_iterator = HfWeightIteratorDirect(
            args=args,
            model=model,
            model_name=model_name,
            quantization_config=quantization_config,
            # An explicit mapping is supplied by disk weight sync from
            # TensorBackuper and is already resident on pinned CPU memory. Keep
            # conversion on CPU instead of staging every bucket back through CUDA.
            target_device="cpu" if megatron_local_weights is not None else None,
        )
    ...
    for chunk_idx, hf_named_tensors in enumerate(
        ...
        if (chunk_idx + 1) % num_save_nodes == 0:
            pending_write = _write_pending_chunk(
                writer,
                pending_write,
                cleanup_cuda=not uses_explicit_cpu_weights,
            )
...
def _write_pending_chunk(
    writer: _SafetensorShardWriter,
    pending_write: tuple[int, Any] | None,
    *,
...
def _finalize_distributed_shards(path: Path, local_state: dict[str, Any]) -> None:
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        states = [None] * dist.get_world_size(group=group)
