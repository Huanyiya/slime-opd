...
from .data import DataIterator, get_data_iterator, log_perf_data, log_rollout_data, to_device_blocking
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        # Register WORLD only when this runtime will actually cycle process
        # groups during train sleep/wake.
        if reload_groups:
        ...
        # Explicitly track whether this process currently owns resident CUDA
        # model memory.  Sequential OPD relies on this invariant because the
        # same physical GPUs are handed to student SGLang, teacher SGLang, and
        # Megatron in turn.
    ...
    def sleep(self) -> None:
        ...
        # torch_memory_saver may enqueue device-to-host copies on a CUDA
        # stream.  Do not let the next colocated SGLang phase reuse the device
        # while those copies are still in flight.  This synchronization is
        # particularly important on PPU, whose driver otherwise can segfault
        # in cmdSemaphoreWait during a later phase transition.
    ...
    def wake_up(self) -> None:
        ...
        # resume() restores parameters with asynchronous host-to-device
        # copies.  NCCL WORLD and the Megatron subgroups must not be recreated
        # until every local copy has completed; overlapping the two caused all
        # PPU ranks to crash in cudaMemcpyAsync/cmdSemaphoreWait.
        torch.cuda.synchronize()

        clear_memory()
        reload_process_groups()
        # resume() already restores the model that was active when sleep()
        # paused this actor. Avoid redundantly copying the same actor snapshot
        # from pinned CPU memory; that H2D copy can crash the PPU driver after
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        rollout_data["tokens"] = [
            to_device_blocking(t, device=device, dtype=torch.long) for t in rollout_data["tokens"]
        ]
        rollout_data["loss_masks"] = [
            to_device_blocking(t, device=device, dtype=torch.int) for t in rollout_data["loss_masks"]
        ]
        if "rollout_mask_sums" in rollout_data:
            ...
            rollout_data["rollout_mask_sums"] = to_device_blocking(
                rollout_data["rollout_mask_sums"], device=device, dtype=torch.float32
            )
        if "multimodal_train_inputs" in rollout_data:
            ...
            rollout_data["multimodal_train_inputs"] = [
                (
                    {
                        key: to_device_blocking(value, device=device, dtype=value.dtype)
    ...
    def train(self, rollout_id: int, rollout_data_ref: Box, external_data=None):
        ...
        with timer("data_preprocess"):
            ...
            # Keep the phase boundary explicit even when a future field adds a
            # CUDA-side transform: training must not start with H2D work in
            # flight from Ray/pageable host memory.
            torch.cuda.synchronize()
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch, external_data=None) -> dict[str, float] | None:
        ...
        with inverse_timer("train_wait"), timer("train"):
            if self.args.compute_advantages_and_returns and not pure_topk_opd:
            ...
            if pure_topk_opd and self._active_model_tag != "actor":
                self._switch_model("actor")
        ...
        if self.args.log_opd_phase_times_only:
            timer_metrics = Timer().log_dict()
            result = None
            if is_megatron_main_rank():
                result = {
                    "student_logprob_forward_time": float(timer_metrics.get("log_probs", 0.0)),
                    "student_train_time": float(timer_metrics.get("actor_train", 0.0)),
                }
            Timer().reset()
            self.weight_updater.pop_metrics()
            return result
    ...
    def update_weights(self) -> None:
        ...
        if (
            disk_weight_update
            and getattr(self.weight_updater, "requires_model_on_gpu_for_export", False)
            and self.args.offload_train
            and self._train_residency == "cpu"
        ):
            self.wake_up()
            onloaded_for_parallel_disk_export = True

        # Full disk sync uses TensorBackuper's pinned-CPU snapshot for TP1.
        # With TP/PP/EP > 1, the actor is temporarily onloaded above and the
        # exporter uses its live CUDA shards for bucketed collectives.
