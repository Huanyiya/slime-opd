...
OPD_TOKEN_KL_VARIANCES_KEY = "_opd_token_kl_variances"


def _reduce_opd_token_kl_variance_metrics(
    losses_reduced: list[dict], *, device: torch.device, dp_with_cp_group
) -> dict[str, float]:
    """Gather per-sequence variances and return exact step-level statistics."""
    local_values = [
        item[OPD_TOKEN_KL_VARIANCES_KEY]
        for item in losses_reduced
        if item[OPD_TOKEN_KL_VARIANCES_KEY].numel() > 0
    ]
    if local_values:
        local_values = torch.cat(local_values).to(device=device, dtype=torch.float32)
    else:
        local_values = torch.empty(0, device=device, dtype=torch.float32)

    world_size = dist.get_world_size(group=dp_with_cp_group)
    if world_size > 1:
        local_size = torch.tensor([local_values.numel()], dtype=torch.int64, device=device)
        gathered_sizes = [torch.zeros_like(local_size) for _ in range(world_size)]
        dist.all_gather(gathered_sizes, local_size, group=dp_with_cp_group)
        sizes = [int(size.item()) for size in gathered_sizes]
        max_size = max(sizes)
        if max_size > 0:
            padded = torch.zeros(max_size, dtype=local_values.dtype, device=device)
            padded[: local_values.numel()] = local_values
            gathered = [torch.empty_like(padded) for _ in range(world_size)]
            dist.all_gather(gathered, padded, group=dp_with_cp_group)
            values = torch.cat(
                [rank_values[:rank_size] for rank_values, rank_size in zip(gathered, sizes, strict=True)]
            )
        else:
            values = local_values
    else:
    ...
    if values.numel() == 0:
        return {}
    return {
        "opd_token_kl_variance_min": values.min().item(),
        "opd_token_kl_variance_max": values.max().item(),
        "opd_token_kl_variance_p05": torch.quantile(values, 0.05).item(),
        "opd_token_kl_variance_p95": torch.quantile(values, 0.95).item(),
    }
...
def reduce_train_step_metrics(
    ...
    *,
    ...
    Pipeline (1:1 with what the train loop used to do inline):
      1. Reduce each metric's per-mb values locally according to its reduction
         mode (mean/sum/min/max/ratio).
      2. All-reduce across the DP*CP group (``dp_with_cp_group``).
      3. Apply the per-mode divisor / cp_factor for ordinary means:
    ...
    metric_reduce_ops = losses_reduced[0].get("metric_reduce_ops")
    if metric_reduce_ops is None:
        metric_reduce_ops = torch.zeros(len(keys), dtype=torch.int64, device=losses_reduced[0]["values"].device)
    else:
        metric_reduce_ops = metric_reduce_ops.to(dtype=torch.int64)
    assert metric_reduce_ops.numel() == len(keys)

    # 0=ordinary mean, 1=global min, 2=global max, 3=ratio with an
    # explicit denominator. The latter is needed for OPD top-k clip ratio:
    # its denominator is valid response positions times K, not rollouts.
    mean_op, min_op, max_op, ratio_op = 0, 1, 2, 3
    values_by_mb = torch.stack([x["values"] for x in losses_reduced])
    assert values_by_mb.shape[1] == len(keys) + 1

    # ``values[:, 0]`` is only used for the existing ordinary-mean metrics.
    values = torch.empty(len(keys), dtype=values_by_mb.dtype, device=values_by_mb.device)
    metric_denominators_by_mb = torch.stack(
        [
            x.get(
                "metric_reduce_denominators",
                torch.zeros(len(keys), dtype=values_by_mb.dtype, device=values_by_mb.device),
            ).to(device=values_by_mb.device, dtype=values_by_mb.dtype)
            for x in losses_reduced
        ]
    )
    ratio_denominators = torch.zeros(len(keys), dtype=values_by_mb.dtype, device=values_by_mb.device)

    for op, reduce_op in (
        (mean_op, dist.ReduceOp.SUM),
        (ratio_op, dist.ReduceOp.SUM),
        (min_op, dist.ReduceOp.MIN),
        (max_op, dist.ReduceOp.MAX),
    ):
        indices = torch.nonzero(metric_reduce_ops == op, as_tuple=False).flatten()
        if indices.numel() == 0:
            continue
        metric_values = values_by_mb[:, indices + 1]
        if op == min_op:
            reduced_values = metric_values.amin(dim=0)
        elif op == max_op:
            reduced_values = metric_values.amax(dim=0)
        else:
            reduced_values = metric_values.sum(dim=0)
        dist.all_reduce(reduced_values, op=reduce_op, group=dp_with_cp_group)
        values[indices] = reduced_values

        if op == ratio_op:
            reduced_denominators = metric_denominators_by_mb[:, indices].sum(dim=0)
            dist.all_reduce(reduced_denominators, op=dist.ReduceOp.SUM, group=dp_with_cp_group)
            ratio_denominators[indices] = reduced_denominators

    if calculate_per_token_loss:
        num_samples_or_tokens = values_by_mb[:, 0].sum()
        dist.all_reduce(num_samples_or_tokens, group=dp_with_cp_group)
        cp_factor = cp_size
    else:
    ...
    for index, key in enumerate(keys):
        op = metric_reduce_ops[index].item()
        if op == mean_op:
            results[key] = (values[index] * cp_factor / num_samples_or_tokens).item()
        elif op == ratio_op:
            results[key] = (values[index] / ratio_denominators[index].clamp_min(1)).item()
        else:
            results[key] = values[index].item()

    # Sequence-level token-KL variances are variable-length. Gather raw
    # values before taking quantiles; quantiles of microbatch quantiles are
    # not the quantiles of the full actor step.
    if any(OPD_TOKEN_KL_VARIANCES_KEY in item for item in losses_reduced):
        results.update(
            _reduce_opd_token_kl_variance_metrics(
                losses_reduced,
                device=values_by_mb.device,
