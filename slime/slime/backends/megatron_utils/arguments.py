...
def _is_moe_config(hf_config):
    ...
    # The compatibility Qwen3.5 config bundled with older SGLang inherits
    # Qwen3NextConfig.  As a result, dense Qwen3.5 configs expose default
    # num_experts/moe_intermediate_size fields even though the checkpoint is
    # not MoE.  The model type is authoritative for the Qwen3.5 family.
    ...
    return any(
        getattr(hf_config, attr, None) not in (None, 0)
        for attr in (
            "num_experts",
            "n_routed_experts",
            "num_local_experts",
...
def _hf_validate_args(args, hf_config):
    ...
    is_moe_config = _is_moe_config(hf_config)
    validate_dense_ffn = not is_moe_config or _has_dense_moe_layers(args)
    ...
    for hf_config_name, megatron_config_name, compare_fn in [
        ...
        if hf_config_name == "intermediate_size" and not validate_dense_ffn:
            continue
        if hf_config_name in ("moe_intermediate_size", "shared_expert_intermediate_size") and not is_moe_config:
            continue
