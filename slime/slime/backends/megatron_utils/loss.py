...
from slime.utils.ppo_utils import (
    ...
    calculate_selected_log_probs_and_entropy,
    compute_approx_kl,
    compute_cispo_loss,
    compute_gspo_kl,
    compute_opsm_mask,
...
_METRIC_REDUCE_OPS_KEY = "_metric_reduce_ops"
_METRIC_REDUCE_DENOMINATORS_KEY = "_metric_reduce_denominators"
_METRIC_REDUCE_MEAN = 0
_METRIC_REDUCE_MIN = 1
_METRIC_REDUCE_MAX = 2
_METRIC_REDUCE_RATIO = 3


def _compute_per_sequence_token_kl_variances(
    per_token_kl: torch.Tensor,
    total_lengths: list[int],
    response_lengths: list[int],
    loss_masks: list[torch.Tensor],
) -> torch.Tensor:
    """Return one population variance per sequence with at least one valid token.

    ``per_token_kl`` follows the local CP response layout. We reduce the
    sufficient statistics (sum, squared sum, count) across CP ranks so the
    variance is over the full response, then retain values on CP rank 0 only
    to avoid duplicates in the later DP*CP gather.
    """
    local_masks = [
        torch.as_tensor(
        ...
        for mask, total_length, response_length in zip(
            loss_masks, total_lengths, response_lengths, strict=True
        )
    ]
    local_lengths = [mask.numel() for mask in local_masks]
    local_token_kls = per_token_kl.detach().split(local_lengths, dim=0)

    stats = []
    for token_kls, mask in zip(local_token_kls, local_masks, strict=True):
        valid = mask.bool()
        valid_token_kls = token_kls[valid]
        stats.append(
            torch.stack(
                [
                    valid_token_kls.sum(),
                    valid_token_kls.square().sum(),
                    per_token_kl.new_tensor(valid_token_kls.numel()),
                ]
            )
        )

    if not stats:
        return per_token_kl.new_empty((0,))

    stats = torch.stack(stats)
    cp_size = mpu.get_context_parallel_world_size()
    if cp_size > 1:
        dist.all_reduce(stats, group=mpu.get_context_parallel_group())

    valid_sequences = stats[:, 2] > 0
    counts = stats[valid_sequences, 2]
    means = stats[valid_sequences, 0] / counts
    variances = stats[valid_sequences, 1] / counts - means.square()
    variances = variances.clamp_min(0)

    if cp_size > 1 and mpu.get_context_parallel_rank() != 0:
        return per_token_kl.new_empty((0,))
    return variances.detach()


def _clip_k1_opd_delta(delta: torch.Tensor, args: Namespace) -> torch.Tensor:
    """Clip only the k1 OPD log-probability difference, when configured."""
    clip = getattr(args, "opd_k1_diff_clip", None)
    if getattr(args, "opd_kl_loss_type", "k1") == "k1" and clip is not None:
        return delta.clamp(min=-clip, max=clip)
    return delta


def _append_k1_opd_clip_metrics(
    log: dict[str, torch.Tensor], delta: torch.Tensor, valid_mask: torch.Tensor, args: Namespace
) -> None:
    """Append pre-clip k1 extrema and a token-entry weighted clip ratio.

    For top-k OPD, ``delta`` has shape [response positions, K]. Boolean
    indexing with the [response positions] mask leaves K entries per valid
    response token, so the ratio denominator is exactly valid_tokens * K.
    """
    clip = getattr(args, "opd_k1_diff_clip", None)
    if getattr(args, "opd_kl_loss_type", "k1") != "k1" or clip is None:
        return

    valid_delta = delta[valid_mask.bool()]
    if valid_delta.numel() == 0:
        clip_count = delta.new_zeros(())
        valid_count = delta.new_zeros(())
        delta_min = delta.new_full((), float("inf"))
        delta_max = delta.new_full((), float("-inf"))
    else:
        clip_count = (valid_delta.abs() > clip).sum().to(dtype=delta.dtype)
        valid_count = torch.tensor(valid_delta.numel(), dtype=delta.dtype, device=delta.device)
        delta_min = valid_delta.min()
        delta_max = valid_delta.max()

    existing_metric_count = len(log)
    log["opd_k1_diff_clip_ratio"] = clip_count.detach()
    log["opd_k1_diff_preclip_min"] = delta_min.detach()
    log["opd_k1_diff_preclip_max"] = delta_max.detach()
    log[_METRIC_REDUCE_OPS_KEY] = torch.tensor(
        [_METRIC_REDUCE_MEAN] * existing_metric_count
        + [_METRIC_REDUCE_RATIO, _METRIC_REDUCE_MIN, _METRIC_REDUCE_MAX],
        dtype=torch.int64,
        device=delta.device,
    )
    log[_METRIC_REDUCE_DENOMINATORS_KEY] = torch.cat(
        [
            torch.zeros(existing_metric_count, dtype=delta.dtype, device=delta.device),
            valid_count.reshape(1),
            torch.zeros(2, dtype=delta.dtype, device=delta.device),
        ]
    )
...
def _build_response_aligned_topk_ids(
    T: int,
    device: torch.device,
    ...
    """Align response-space ``[R,K]`` IDs to the local packed-logits layout."""
    if not topk_ids:
        raise ValueError("Top-K OPD requires a non-empty opd_topk_token_ids batch.")
    top_k = topk_ids[0].shape[1]
    normalized_ids: list[torch.Tensor] = []
    for ids, response_length in zip(topk_ids, response_lengths, strict=True):
        ids = ids.to(device=device, dtype=torch.long)
        if ids.ndim != 2 or tuple(ids.shape) != (response_length, top_k):
            raise ValueError(
                f"opd_topk_token_ids must have shape ({response_length}, {top_k}), got {tuple(ids.shape)}."
            )
        normalized_ids.append(ids)

    cp_size = mpu.get_context_parallel_world_size()
    if cp_size > 1 and not allgather_cp:
        aligned = torch.zeros((T, top_k), dtype=torch.long, device=device)
        packed_offset = 0
        for ids, total_length, response_length in zip(
            normalized_ids, total_lengths, response_lengths, strict=True
        ):
            prompt_length = total_length - response_length
            chunk_size, chunks_offset, logits_offset, tokens_offset = get_logits_and_tokens_offset_with_cp(
                total_length, response_length
            )
            for half, base in ((0, packed_offset), (1, packed_offset + chunk_size)):
                local_start = logits_offset[half][0] - chunks_offset[half][0]
                local_end = logits_offset[half][1] - chunks_offset[half][0]
                response_start = tokens_offset[half][0] - prompt_length
                response_end = tokens_offset[half][1] - prompt_length
                aligned[base + local_start : base + local_end] = ids[response_start:response_end]
            packed_offset += 2 * chunk_size
        return aligned

    global_T = sum(total_lengths) if allgather_cp else T
    aligned = torch.zeros((global_T, top_k), dtype=torch.long, device=device)
    sequence_offset = 0
    for ids, total_length, response_length in zip(normalized_ids, total_lengths, response_lengths, strict=True):
        prompt_length = total_length - response_length
        if prompt_length <= 0:
            raise ValueError(f"Top-K OPD requires a non-empty prompt, got prompt length {prompt_length}.")
        response_logit_start = sequence_offset + prompt_length - 1
        response_logit_end = sequence_offset + total_length - 1
        aligned[response_logit_start:response_logit_end] = ids
        sequence_offset += total_length

    if allgather_cp:
        cp_rank = mpu.get_context_parallel_rank()
        chunk_start = cp_rank * T
        chunk_end = chunk_start + T
        if chunk_end <= global_T:
            return aligned[chunk_start:chunk_end].contiguous()
        local = torch.zeros((T, top_k), dtype=torch.long, device=device)
        valid = global_T - chunk_start
        if valid > 0:
            local[:valid] = aligned[chunk_start:]
        return local
    return aligned
...
def apply_opd_kl_to_advantages(
    ...
    Computes the configured OPD KL estimator from student and teacher log-probs
    and adds its weighted penalty to advantages in-place. This is orthogonal to
    the base advantage estimator.
    ...
    opd_kls = []
    opd_kl_loss_type = getattr(args, "opd_kl_loss_type", "k1")
    for i, adv in enumerate(advantages):
        raw_opd_kl = compute_approx_kl(
            student_log_probs[i],
            teacher_log_probs[i],
            kl_loss_type=opd_kl_loss_type,
        )
        opd_kl = _clip_k1_opd_delta(raw_opd_kl, args)
        advantages[i] = adv - args.opd_kl_coef * opd_kl
        # Keep the raw estimator for pre-clip logging in policy_loss_function.
        opd_kls.append(raw_opd_kl)

    # Keep the metric key stable for existing dashboards. For k3 it stores the
    # selected OPD KL estimator rather than the raw log-probability difference.
    rollout_data["opd_reverse_kl"] = opd_kls
...
def compute_advantages_and_returns(args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    if args.use_opd and args.opd_loss_type == "sampled":
...
def opd_topk_loss_function(
    ...
    """Compute pure student-Top-K reverse KL without entering the PPO path."""
    topk_ids = batch.get("opd_topk_token_ids")
    teacher_topk_log_probs = batch.get("opd_topk_teacher_log_probs")
    if topk_ids is None or teacher_topk_log_probs is None:
        raise ValueError(
            "Top-K OPD requires opd_topk_token_ids and opd_topk_teacher_log_probs in the training batch."
        )
    ...
    student_topk_log_probs, student_entropy = _extract_per_sample(
        selected_log_probs,
        entropy_full,
        total_lengths,
        response_lengths,
    ...
    if args.allgather_cp:
        ...
        student_topk_log_probs = response_data["student_topk_log_probs"]
    ...
    for position, (student_log_probs, teacher_log_probs) in enumerate(
        zip(student_topk_log_probs, local_teacher_topk_log_probs, strict=True)
    ):
        if student_log_probs.shape != teacher_log_probs.shape:
            raise ValueError(
                f"Student/teacher Top-K logprob shape mismatch for sample {position}: "
                f"{tuple(student_log_probs.shape)} vs {tuple(teacher_log_probs.shape)}."
            )
        if student_log_probs.ndim != 2 or student_log_probs.shape[1] != args.opd_top_k:
            raise ValueError(
                f"Top-K OPD expected local [R,{args.opd_top_k}] logprobs, got {tuple(student_log_probs.shape)}."
            )
...
def policy_loss_function(
    ...
    reported_loss = {
        ...
        "student_entropy": entropy_loss.clone().detach(),
    ...
    if "opd_reverse_kl" in batch:
        ...
        _append_k1_opd_clip_metrics(
            reported_loss,
            opd_reverse_kl,
            torch.cat(batch["loss_masks"], dim=0),
            args,
        )
...
def loss_function(
    ...
    token_kl_variances = log.pop(OPD_TOKEN_KL_VARIANCES_KEY, None)
    metric_reduce_ops = log.pop(_METRIC_REDUCE_OPS_KEY, None)
    metric_reduce_denominators = log.pop(_METRIC_REDUCE_DENOMINATORS_KEY, None)
    if metric_reduce_ops is None:
        metric_reduce_ops = torch.full(
            (len(log),), _METRIC_REDUCE_MEAN, dtype=torch.int64, device=logits.device
        )
    if metric_reduce_denominators is None:
        metric_reduce_denominators = torch.zeros(len(log), dtype=logits.dtype, device=logits.device)
    assert metric_reduce_ops.numel() == len(log)
    assert metric_reduce_denominators.numel() == len(log)
    ...
    logging_output = {
        "keys": list(log.keys()),
        "metric_reduce_ops": metric_reduce_ops,
        "metric_reduce_denominators": metric_reduce_denominators,
    ...
    if token_kl_variances is not None:
        logging_output[OPD_TOKEN_KL_VARIANCES_KEY] = token_kl_variances
