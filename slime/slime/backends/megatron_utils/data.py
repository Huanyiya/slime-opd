...
def to_device_blocking(value: torch.Tensor, *, device, dtype) -> torch.Tensor:
    """Copy a CPU tensor to a device without the pageable-memory async path.

    Ray-deserialized rollout tensors are generally not pinned.  On PPU,
    ``non_blocking=True`` for such tensors enters the driver's staged H2D path,
    which can segfault after a torch-memory-saver/NCCL phase transition.
    """
    return value.to(device=device, dtype=dtype, non_blocking=False)
...
def get_batch(
    ...
    # Top-K OPD metadata stays on CPU in rollout_data. Move only the current
    # micro-batch to GPU so long responses do not pin a second full batch there.
    device = torch.cuda.current_device()
    if batch.get("opd_topk_token_ids") is not None:
    ...
    if batch.get("opd_topk_teacher_log_probs") is not None:
        batch["opd_topk_teacher_log_probs"] = [
            ...
            for value in batch["opd_topk_teacher_log_probs"]
        ]
...
def gather_log_data(
    ...
    if reduced is None:
        return None
    display_key_map = {"log_probs": "_student_forward_log_probs"} if metric_name == "rollout" else {}
    reduced_log_dict = {f"{metric_name}/{display_key_map.get(k, k)}": v for k, v in reduced.items()}
    step = get_metric_train_step(args, fallback=compute_first_train_step_for_rollout(args, rollout_id))
    add_train_step_metric(reduced_log_dict, step)
    logger.info(f"{metric_name} {rollout_id}: {reduced_log_dict}")
    logging_utils.log(args, reduced_log_dict, step_key="step")
    ...
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        ...
        if args.ci_test and reduced_log_dict is not None:
            ...
            if (
                ...
                and "rollout/_student_forward_log_probs" in reduced_log_dict
                ...
                assert (
                    abs(reduced_log_dict["rollout/_student_forward_log_probs"] - reduced_log_dict["rollout/ref_log_probs"])
                    < 1e-8
                )
            if "rollout/_student_forward_log_probs" in reduced_log_dict:
                assert -1 < reduced_log_dict["rollout/_student_forward_log_probs"] < 0
