...
class _VocabParallelSelectedLogProbs(torch.autograd.Function):
    """Exact full-vocabulary log-probs and optional entropy per position.

    Full-vocabulary probabilities are only transient scratch tensors. Backward
    recomputes softmax from the saved logits instead of retaining an extra
    ``[T, V_local]`` probability buffer across the forward/backward boundary.
    """
    ...
    def forward(
        ...
        vocab_parallel_logits = vocab_parallel_logits.float()
        _seq_len, vocab_parallel_size = vocab_parallel_logits.shape
        rank, _world_size = _get_vocab_parallel_rank_size(process_group)
        vocab_start_index = rank * vocab_parallel_size
        vocab_end_index = vocab_start_index + vocab_parallel_size

        target_mask = (target_ids < vocab_start_index) | (target_ids >= vocab_end_index)
        local_target_ids = (target_ids - vocab_start_index).masked_fill(target_mask, 0)

        logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
        _maybe_all_reduce(logits_max, dist.ReduceOp.MAX, process_group)
        normalized_logits = vocab_parallel_logits - logits_max
        selected_logits = normalized_logits.gather(dim=-1, index=local_target_ids).masked_fill_(target_mask, 0.0)
        _maybe_all_reduce(selected_logits, dist.ReduceOp.SUM, process_group)

        # Consume the full-vocabulary scratch buffer in place. Only logits and
        # small [T,K] metadata survive until backward.
        exp_logits = normalized_logits.exp_()
        sum_exp_logits = exp_logits.sum(dim=-1, keepdim=True)
        _maybe_all_reduce(sum_exp_logits, dist.ReduceOp.SUM, process_group)
        selected_log_probs = selected_logits - sum_exp_logits.log()

        entropy = vocab_parallel_logits.new_zeros((0,))
        if with_entropy:
            # H(p) = log Z - E_p[logit]. ``exp_logits`` is transient scratch;
            # only the per-position entropy survives this forward.
            sum_exp_times_logits = torch.einsum("ij,ij->i", exp_logits, vocab_parallel_logits).unsqueeze(-1)
            _maybe_all_reduce(sum_exp_times_logits, dist.ReduceOp.SUM, process_group)
            entropy = logits_max + sum_exp_logits.log() - sum_exp_times_logits / sum_exp_logits
            entropy = entropy.squeeze(dim=-1)
            ctx.mark_non_differentiable(entropy)

        ctx.process_group = process_group
        ctx.save_for_backward(vocab_parallel_logits, target_mask, local_target_ids)
        return selected_log_probs, entropy

    @staticmethod
    def backward(ctx, grad_selected_log_probs: torch.Tensor, _grad_entropy: torch.Tensor | None):
        vocab_parallel_logits, target_mask, local_target_ids = ctx.saved_tensors
        process_group = ctx.process_group

        logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
        _maybe_all_reduce(logits_max, dist.ReduceOp.MAX, process_group)
        softmax = (vocab_parallel_logits - logits_max).exp_()
        sum_exp_logits = softmax.sum(dim=-1, keepdim=True)
        _maybe_all_reduce(sum_exp_logits, dist.ReduceOp.SUM, process_group)
        softmax.div_(sum_exp_logits)

        grad_input = -softmax * grad_selected_log_probs.sum(dim=-1, keepdim=True)
        local_updates = grad_selected_log_probs.masked_fill(target_mask, 0.0)
        grad_input.scatter_add_(dim=-1, index=local_target_ids, src=local_updates)
        return grad_input, None, None, None


def _calculate_selected_log_probs_chunk(
    logits: torch.Tensor,
    target_ids: torch.Tensor,
    tp_group,
    *,
    ...
    selected_log_probs, entropy = _VocabParallelSelectedLogProbs.apply(logits, target_ids, tp_group, with_entropy)
    if not with_entropy:
        entropy = None
    return selected_log_probs, entropy
...
def calculate_selected_log_probs(
    logits: torch.Tensor,
    target_ids: torch.Tensor,
    tp_group,
    chunk_size: int = -1,
) -> torch.Tensor:
    """Compute full-vocabulary-normalized log-probs for ``target_ids[T,K]``."""
    if logits.ndim != 2 or target_ids.ndim != 2:
        raise ValueError(f"Expected logits [T,V] and target_ids [T,K], got {logits.shape} and {target_ids.shape}.")
    if logits.size(0) != target_ids.size(0):
        raise ValueError(f"Logits and target_ids row counts differ: {logits.size(0)} vs {target_ids.size(0)}.")

    logits = logits.contiguous()
    target_ids = target_ids.to(device=logits.device, dtype=torch.long).contiguous()
    selected_log_probs, _ = calculate_selected_log_probs_and_entropy(
        logits,
        target_ids,
        tp_group,
...
def calculate_selected_log_probs_and_entropy(
    logits: torch.Tensor,
    target_ids: torch.Tensor,
    tp_group,
    chunk_size: int = -1,
    *,
    ...
    """Compute selected log-probs and metric-only full-vocabulary entropy."""
    if logits.ndim != 2 or target_ids.ndim != 2:
        raise ValueError(f"Expected logits [T,V] and target_ids [T,K], got {logits.shape} and {target_ids.shape}.")
    if logits.size(0) != target_ids.size(0):
        raise ValueError(f"Logits and target_ids row counts differ: {logits.size(0)} vs {target_ids.size(0)}.")
    ...
    target_ids = target_ids.to(device=logits.device, dtype=torch.long).contiguous()
    if logits.size(0) == 0:
        selected_log_probs = logits.sum(dim=-1, keepdim=True).expand(-1, target_ids.size(1))
    ...
    if chunk_size > 0:
        num_chunks = (logits.size(0) - 1) // chunk_size + 1
        outputs = [
            _calculate_selected_log_probs_chunk(
                logits_chunk,
                target_chunk,
                tp_group,
                with_entropy=with_entropy,
            )
            for logits_chunk, target_chunk in zip(
                logits.chunk(num_chunks, dim=0),
                target_ids.chunk(num_chunks, dim=0),
                strict=True,
            )
        ]
        selected_log_probs = torch.cat([output[0] for output in outputs], dim=0)
        entropy = torch.cat([output[1] for output in outputs], dim=0) if with_entropy else None
        return selected_log_probs, entropy
    return _calculate_selected_log_probs_chunk(logits, target_ids, tp_group, with_entropy=with_entropy)
