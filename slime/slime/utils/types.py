...
def _extract_opd_topk_data(
    ...
    *,
    ...
    if len(output_top_logprobs) != expected_num_tokens:
        raise ValueError(
            "SGLang output_top_logprobs length must equal generated token count: "
            f"got={len(output_top_logprobs)}, expected={expected_num_tokens}."
        )

    id_rows: list[list[int]] = []
    logprob_rows: list[list[float]] = []
    for position, entries in enumerate(output_top_logprobs):
        if len(entries) != top_k:
            raise ValueError(
                f"SGLang output_top_logprobs[{position}] must contain {top_k} entries, got {len(entries)}."
            )
        try:
            id_rows.append([int(entry[1]) for entry in entries])
            logprob_rows.append([float(entry[0]) for entry in entries])
...
class Sample:
    ...
    def append_response_tokens(
        ...
        *,
        ...
        should_pad_response_metadata = bool(tokens and not trainable)
        if meta_info is not None or should_pad_response_metadata:
            self._apply_meta_info(
                args,
                meta_info or {},
                new_token_count=len(tokens),
                pad_missing_top_p=should_pad_response_metadata,
                pad_missing_opd_topk=should_pad_response_metadata,
    ...
    def _apply_meta_info(
        ...
        *,
        ...
        pad_missing_opd_topk: bool = False,
        update_terminal_info: bool = True,
    ) -> None:
        ...
        if use_opd_topk and new_token_count:
            ...
            if pad_missing_opd_topk:
                ...
                for metric_name, probability_sum in student_probability_sums.items():
                    numerator_key = f"{metric_name}_numerator"
                    denominator_key = f"{metric_name}_denominator"
                    self.metadata[numerator_key] = float(self.metadata.get(numerator_key, 0.0)) + probability_sum
                    self.metadata[denominator_key] = int(self.metadata.get(denominator_key, 0)) + new_token_count
            if self.opd_topk_token_ids is None:
                self.opd_topk_token_ids = new_topk_ids
            else:
                existing_topk_ids = torch.as_tensor(self.opd_topk_token_ids, dtype=torch.int32)
                if existing_topk_ids.ndim != 2 or existing_topk_ids.shape[1] != top_k:
                    raise ValueError(
                        "Existing opd_topk_token_ids has incompatible shape: "
    ...
    def _validate_response_metadata_lengths(self):
        ...
        if self.rollout_top_p_token_ids is not None or self.rollout_top_p_token_offsets is not None:
        ...
        if self.opd_topk_token_ids is not None:
            topk_ids = torch.as_tensor(self.opd_topk_token_ids)
            if topk_ids.ndim != 2 or topk_ids.shape[0] != self.response_length:
                raise ValueError(
