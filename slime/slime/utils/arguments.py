    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "For OPD runs, suppress detailed perf metrics and log only coarse per-step phase timings: "
                    "student rollout, teacher score, student logprob forward, student train, weight sync, and total."
                ),
            )
        ...
        def add_eval_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default=None,
                help="Custom reward function used only for evaluation datasets.",
            )
            parser.add_argument(
            ...
            parser.add_argument("--eval-min-p", type=float, default=None)
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            Sampled-token OPD adds a KL penalty to advantages. Student Top-K OPD
            uses a standalone distillation loss during the actor update.
            """
            parser.add_argument(
                "--use-opd",
                action="store_true",
            ...
            parser.add_argument(
                ...
                choices=["k1", "k2", "k3"],
                default="k1",
                help="KL estimator for the OPD penalty. k1 preserves the previous objective.",
            )
            parser.add_argument(
                "--opd-k1-diff-clip",
                type=float,
                default=None,
                help=(
                    "Optional symmetric clip value for the per-token k1 OPD log-probability difference "
                    "(student_log_probs - teacher_log_probs). Disabled when unset."
                ),
            )
            parser.add_argument(
                "--opd-loss-type",
            ...
            parser.add_argument(
                ...
                type=int,
                default=16,
                help="Number of student Top-K tokens per response position for --opd-loss-type=topk.",
            )
            parser.add_argument(
                "--opd-teacher-load",
                type=str,
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                "--sequential-opd-teacher-model-path",
                type=str,
                default=None,
                help=(
                    "Run an SGLang OPD teacher after student rollout on the same colocated GPU pool. "
                    "Student generation, teacher scoring, and Megatron update execute sequentially."
                ),
            )
            parser.add_argument(
                "--sequential-opd-teacher-num-gpus-per-engine",
                type=int,
                default=1,
                help="Tensor-parallel GPU count per frozen teacher engine in sequential OPD mode.",
            )
            parser.add_argument(
                "--sequential-opd-teacher-mem-fraction-static",
                type=float,
                default=0.7,
                help="SGLang static-memory fraction for the sequential frozen teacher.",
            )
            parser.add_argument(
                "--sequential-opd-teacher-chunked-prefill-size",
                type=int,
                default=6144,
                help="Chunked-prefill token count used by the sequential frozen teacher.",
            )
            parser.add_argument(
                "--custom-rm-path",
                type=str,
...
def slime_validate_args(args):
    ...
    if args.use_opd:
        ...
        if args.opd_k1_diff_clip is not None and args.opd_k1_diff_clip <= 0:
            raise ValueError(f"--opd-k1-diff-clip must be positive, got {args.opd_k1_diff_clip}.")
    ...
    if getattr(args, "sequential_opd_teacher_model_path", None) is not None:
        assert args.colocate, "--sequential-opd-teacher-model-path requires --colocate."
        assert args.offload_rollout and args.offload_train, (
            "Sequential OPD requires both rollout and train memory offload."
        )
        assert not args.rollout_external, "Sequential OPD does not support external rollout engines."
        assert args.custom_rm_path is not None, "Sequential OPD requires --custom-rm-path."
        assert not args.group_rm, "Sequential OPD currently supports per-sequence teacher scoring only."
        assert args.dynamic_sampling_filter_path is None, (
            "Sequential OPD defers teacher rewards until rollout finishes, so reward-based dynamic sampling "
            "cannot run during generation."
        )
        assert args.sequential_opd_teacher_num_gpus_per_engine > 0
        assert 0 < args.sequential_opd_teacher_mem_fraction_static < 1
        assert args.sequential_opd_teacher_chunked_prefill_size > 0
        assert args.rollout_num_gpus % args.sequential_opd_teacher_num_gpus_per_engine == 0, (
            "rollout_num_gpus must be divisible by sequential_opd_teacher_num_gpus_per_engine."
        )
        if args.rm_url is not None:
            logger.info("Ignoring --rm-url: sequential OPD uses its Ray-managed teacher router.")
