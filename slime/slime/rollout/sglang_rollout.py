...
async def generate(
    ...
    metadata_args = args
    if use_opd_topk and not collect_opd_topk:
        metadata_args = copy.copy(args)
        metadata_args.opd_loss_type = "sampled"
...
async def generate_and_rm(
    ...
    defer_teacher_reward = bool(getattr(args, "sequential_opd_teacher_model_path", None)) and not evaluation
    ...
    if sample.status == Sample.Status.COMPLETED or sample.status == Sample.Status.TRUNCATED:
        ...
        if not args.group_rm and not defer_teacher_reward:
    ...
    # In sequential OPD mode the student must finish using the shared GPUs
    # before the frozen teacher is resumed. RolloutManager scores the complete
    # batch after switching the two SGLang servers.
    if defer_teacher_reward:
        return sample
...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        ...
        for task in done:
            ...
            if do_print:
                ...
                logger.info(
                    "First rollout sample completed: response_length=%d, status=%s, reward=%s",
    ...
    logger.info(
        "Finish rollout: response_length=%d, status=%s, reward=%s",
        sample.response_length,
        sample.status,
        sample.reward,
    )
...
async def eval_rollout(args: Namespace, rollout_id: int) -> tuple[dict[str, dict[str, list[Any]]], list[list[Sample]]]:
    ...
    stream_eval_rollouts = bool(os.environ.get("OPD_EVAL_ROLLOUT_PATH"))
    if stream_eval_rollouts:
        from slime.rollout.on_policy_distillation import _reset_eval_rollout_stream
    ...
    try:
        ...
        if stream_eval_rollouts:
            from slime.rollout.on_policy_distillation import _close_eval_rollout_stream
...
async def eval_rollout_single_dataset(
    ...
    for coro in asyncio.as_completed(tasks):
        sample = await coro
        completed_samples = sample if isinstance(sample, list) else [sample]
        if os.environ.get("OPD_EVAL_ROLLOUT_PATH"):
            from slime.rollout.on_policy_distillation import _append_eval_rollout_stream
