...
def last_boxed_only_string(string):
    def find_last_complete(command):
        search_end = len(string)
        while True:
            idx = string.rfind(command, 0, search_end)
            if idx < 0:
                return None

            left_brace_idx = idx + len(command)
            if left_brace_idx >= len(string) or string[left_brace_idx] != "{":
                search_end = idx
                continue

            num_left_braces_open = 0
            for i in range(left_brace_idx, len(string)):
                if string[i] == "{":
            ...
            # The rightmost occurrence is incomplete, usually because rollout
            # generation was truncated. Continue searching earlier occurrences.
            search_end = idx

    boxed = find_last_complete("\\boxed")
    if boxed is not None:
        return boxed
    return find_last_complete("\\fbox")
